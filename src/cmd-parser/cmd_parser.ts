...
export class ShowMdCmdParser extends EventEmitter {
  ...
  parse(argv: string[]): ShowMdCmdParser {
    ...
    if (args._[0] === undefined || args._[0] === '') return this.cmdEmpty(args);
    switch (args._[0].toLowerCase()) {
      ...
      default:
        this.emit(
          ...
          'Unknown command "' +
            args._[0] +
            '"! Enter help for more detailed information.\n'
        );
        return this;
    }
  }
  ...
  Mandatory arguments to long options are mandatory for short options too.
    ...
    if (args.dir !== undefined)
      this.app.config.setPath(this.app.config.SERVER_ROOT, args.dir);
  ...
  Mandatory arguments to long options are mandatory for short options too.
    ...
    -s, -- style <path | theme>       set custom stylesheet or a predefined theme.
      ...
      this.emit(
        'warning',
        'Server already running on http://localhost:' +
          this.app.config.getPort()
      );
    } else {
      if (args.dir !== undefined)
        this.app.config.setPath(this.app.config.SERVER_ROOT, args.dir);
  ...
  cmdStop(args: ParsedArgs): ShowMdCmdParser {
    if (!this.app.isRunning()) this.emit('warning', 'Server is not running.');
    else this.app.stop();
    return this;
  }
