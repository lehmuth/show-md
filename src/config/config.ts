...
export class ShowMdConfig extends EventEmitter {
  ...
  constructor() {
    super();
    let defaultConfig = require(__dirname + '/default.json');
    ...
    this.includeExtensions = defaultConfig.includeExtensions ?? [
      '.html',
      '.md',
      '.txt',
    ];
    ...
    this.htDirs = defaultConfig.htDirs ?? ['{SERVER_ROOT}', '{HTDOCS}'];
  ...
  replacePathVariables(target: string): string {
    return path.resolve(
      target.replace(/^{(.*)}/, (a: string, variable: string): string => {
  ...
  getDefinedStylesheets(): string[] {
    return this.stylesheets.map((elem) => {
      return elem[0];
    });
  }
